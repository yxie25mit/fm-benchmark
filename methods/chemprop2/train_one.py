"""
Chemprop v2 train_one — wraps `chemprop train` CLI from chemprop2 conda env.

Identical CLI contract to other methods' train_one.py:
...
# Default HPs — chemprop v2 CLI defaults (paper defaults for D-MPNN).
# Aggregation set to "norm" + aggregation_norm=100 to match prior hyperopt_v2_r7
# tuned chemprop runs (the "paper-baseline" configuration).
DEFAULT_HP = {
    "depth":             3,
...
def compute_val_metric(out, scratch_root, dataset, protocol, seed, targets, task_type,
    ...
    SAME per-target NaN-aware macro aggregation used for test. This exposes a
    val metric for HP selection (pick_best_hp) instead of leaking via test.
    Returns the aggregated val metric, or None if anything fails."""
...
def main():
    ...
    if (out / "done.flag").exists():
        print(f"[chemprop2] already done: {out}")
        return
    ...
    # the final artifacts.
    scratch_root = make_scratch_root("CHEMPROP2_SCRATCH", "chemprop2")
    ...
    cmd = [
        ...
        "--splits-column", "split",
        "--depth", str(hp["depth"]),
        "--message-hidden-dim", str(hp["message_hidden_dim"]),
        ...
        "--aggregation", str(hp["aggregation"]),
    ...
    # Respect parent's CUDA_VISIBLE_DEVICES (orchestrator masks to one GPU).
    # Only set if running standalone. Set on this process so the in-process val
    # prediction below sees the same masking as the training subprocess.
    if "CUDA_VISIBLE_DEVICES" not in os.environ:
        os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu)
    env = os.environ.copy()

    print(f"[chemprop2] launching: {' '.join(cmd[:10])}...")
    ...
    if result.returncode != 0:
        ...
        print(f"[chemprop2] FAILED returncode={result.returncode}; see {out}/stderr.log")
        return
    ...
    if not pred_files:
        print(f"[chemprop2] no predictions found under {scratch_root}/cp_out")
        return
    ...
    print(f"[chemprop2] DONE: {out}  AM={agg_am} GM={agg_gm}  ({elapsed:.0f}s)")
